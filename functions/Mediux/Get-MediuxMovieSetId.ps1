...
"@
    ...
    try {
        ...
        Log-Message -Type "WRN" -Message "Failed to fetch data from Mediux GraphQL: $_"
        return $null
    }

    $movie = $response.data.movies_by_id
    if (-not $movie) {
        Log-Message -Type "INF" -Message "No movie found for TMDB ID $tmdbId"
        return $null
    }
    ...
    function Match-Set {
        ...
        foreach ($preferredUsername in $preferredUsernames) {
            ...
            Log-Message -Type "INF" -Message "Found $($userSets.Count) $source sets by '$preferredUsername'"
            ...
            if ($source -eq "movie") {
                ...
                if ($selected) {
                    Log-Message -Type "WRN" -Message "Partial match with full title: '$($selected.set_title)'"
                    return $selected
                }
                ...
                if ($selected) {
                    Log-Message -Type "WRN" -Message "Matched using cleaned base title: '$($selected.set_title)'"
                    return $selected
                }
                ...
                if ($selected) {
                    Log-Message -Type "INF" -Message "Collection set matched originating title '$title'"
                    return $selected
                }
                ...
                $selected = $filteredUserSets |
                    ...
                    Select-Object -First 1
                if ($selected) {
                    Log-Message -Type "INF" -Message "Collection set matched cleaned title fallback"
                    return $selected
                }
                ...
                $selected = $filteredUserSets |
                    ...
                    Select-Object -First 1
                if ($selected) {
                    Log-Message -Type "INF" -Message "Collection set matched loose collection name fallback"
                    return $selected
                }
            }
        }
    ...
    if ($movieSets.Count -gt 0) {
        Log-Message -Type "INF" -Message" Total movie sets found: $($movieSets.Count)"
        $result = Match-Set -sets:$movieSets -source:"movie"
        if ($result) {
            $assetUrl = "https://api.mediux.pro/assets/$($result.poster_id)"
            Log-Message -Type "SUC" -Message "Selected Movie Set: $($result.set_title) (ID=$($result.id))"
            return @{
                setId    = $result.id
    ...
    if ($collectionSets.Count -gt 0) {
        Log-Message -Type "INF" -Message "Total collection sets found: $($collectionSets.Count)"
        $result = Match-Set -sets:$collectionSets -source:"collection"
        if ($result) {
            $assetUrl = "https://api.mediux.pro/assets/$($result.poster_id)"
            Log-Message -Type "INF" -Message "Selected Collection Set: $($result.set_title) (ID=$($result.id))"
            return @{
                setId    = $result.id
                assetId  = $result.poster_id
                assetUrl = $assetUrl
    ...
    Log-Message -Type "WRN" -Message "No Mediux set matched any uploader or condition."
    return $null
}